...
1. No painel de navegação à esquerda, na página NSG-SC900, em Configurações, selecione **Adaptadores de rede**.
    ...
    1. No campo de seleção das associações do adaptador de rede, selecione a **seta para baixo**, **sc900-winvmXXX** e **OK** na parte inferior da janela. Uma vez que a interface é associada ao NSG, ela aparecerá na lista.
...
1. No painel de navegação à esquerda, selecione **Conectar**.
    1. Você pode simplesmente selecionar **verificar acesso**.  O status deve ser mostrado como "Acessível". Alternativamente, se você tiver tempo, poderá conectar-se à VM abrindo uma instância da Conexão de Área de Trabalho Remota no Windows.  Essa opção abrirá a VM ao conectar-se a ela com êxito.  Abaixo estão listadas as etapas:
        ...
        1. Uma janela da conexão da Área de Trabalho Remota será aberta indicando: Não foi possível verificar a identidade do computador remoto. Você deseja se conectar mesmo assim? Selecione **Sim**.
